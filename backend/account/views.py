...
class RegisterSerializer(serializers.ModelSerializer):
    username = serializers.CharField(write_only=True) # Frontend sends 'username' for user_id
    ...
    def create(self, validated_data):
        # Map 'username' from frontend to 'user_id' and 'nickname' in CustomUser
        # 'username' argument is required by default UserManager
        user = User.objects.create_user(
            username=validated_data['username'], 
            user_id=validated_data['username'],
            password=validated_data['password'],
            nickname=validated_data['username'] # Default nickname to same as user_id for now
        )
        return user
...
class RegisterView(views.APIView):
    ...
    authentication_classes = [] # Disable session auth for this view to avoid CSRF
...
class LoginView(views.APIView):
    permission_classes = [AllowAny]
    authentication_classes = [] # Disable session auth for this view

    def post(self, request):
        serializer = LoginSerializer(data=request.data)
        if serializer.is_valid():
            user = authenticate(
                request, # Pass request for custom backends if needed
                username=serializer.validated_data['username'],
                password=serializer.validated_data['password']
            )
            if user is not None:
                login(request, user) # Optional: keep session login if needed
                return Response({'message': 'Login successful', 'username': user.nickname}, status=status.HTTP_200_OK)
